  ...
  return (
    <div>
      ...
      {loading ? <div className="spinner" /> : (
        <div className="card"><div style={{ overflowX: 'auto' }}><table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 14 }}>
          ...
          <tbody>{items.map(s => <tr key={s.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
            <td style={{ padding: 12, fontWeight: 600 }}>{s.name}</td><td style={{ padding: 12 }}>{s.representativeName || '-'}</td><td style={{ padding: 12 }}>{s.email || '-'}</td><td style={{ padding: 12 }}>{s.phone || '-'}</td>
            <td style={{ padding: 12 }}><span className={`badge ${s.active ? 'badge-success' : 'badge-secondary'}`}>{s.active ? 'Hoạt động' : 'Tắt'}</span></td>
            <td style={{ padding: 12 }}><button className="btn btn-secondary btn-sm" onClick={() => openEdit(s)}><i className="fa-solid fa-pen" /></button> <button className="btn btn-danger btn-sm" onClick={() => remove(s.id)}><i className="fa-solid fa-trash" /></button></td>
          </tr>)}</tbody>
        </table></div></div>
      )}
      {modal && <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.45)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
