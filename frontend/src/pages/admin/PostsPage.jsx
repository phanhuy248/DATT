...
export default function AdminPostsPage() {
  ...
  return (
    <div>
      <div className="admin-page-header" style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 20 }}><h1 style={{ fontSize: 22, fontWeight: 700 }}>Quản lý tin tức</h1><button className="btn btn-primary" onClick={openCreate}><i className="fa-solid fa-plus" /> Thêm tin</button></div>
      {loading ? <div className="spinner" /> : <div className="card"><div className="admin-table-wrap" style={{ overflowX: 'auto' }}><table className="admin-table-card" style={{ width: '100%', borderCollapse: 'collapse', fontSize: 14 }}>
        ...
        <tbody>{items.map(p => <tr key={p.id} style={{ borderBottom: '1px solid #f1f5f9' }}><td data-label="Tiêu đề" style={{ padding: 12, fontWeight: 600 }}>{p.title}</td><td data-label="Slug" style={{ padding: 12 }}>{p.slug}</td><td data-label="Tóm tắt" style={{ padding: 12, maxWidth: 320 }}>{p.summary}</td><td data-label="Trạng thái" style={{ padding: 12 }}><span className={`badge ${p.published ? 'badge-success' : 'badge-secondary'}`}>{p.published ? 'Hiển thị' : 'Ẩn'}</span></td><td data-label="Thao tác" style={{ padding: 12 }}><button className="btn btn-secondary btn-sm" onClick={() => openEdit(p)}><i className="fa-solid fa-pen" /></button> <button className="btn btn-danger btn-sm" onClick={() => remove(p.id)}><i className="fa-solid fa-trash" /></button></td></tr>)}</tbody>
      </table></div></div>}
      {modal && <div className="admin-modal-overlay" style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.45)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 16 }}>
