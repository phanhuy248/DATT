...
function resolveDefaultSalePercent(product) {
...
export default function ProductCard({ product, badge, disableCart = false, salePercent, onUnfavorited }) {
  ...
  let discountPercent = resolveSalePercent(product, oldPrice, requestedSalePercent)
  if (discountPercent === 0) {
    discountPercent = resolveDefaultSalePercent(product)
  }
    ...
    <article className="group relative flex h-full flex-col overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-[0_2px_8px_rgba(0,0,0,0.04)] transition-all duration-300 hover:-translate-y-1.5 hover:border-shop-red hover:shadow-[0_12px_24px_-4px_rgba(215,0,24,0.08)]">
      ...
      <div className="flex flex-1 flex-col p-4">
        ...
        <div className="mb-1.5 flex items-center justify-between">
          <span className="text-[10px] font-extrabold tracking-wider text-shop-red/80 uppercase">
            {product?.factory || 'Chính hãng'}
          </span>
          {product?.categoryName && (
            <span className="rounded bg-slate-100 px-1.5 py-0.5 text-[9px] font-bold text-slate-500 uppercase tracking-tight">
              {product.categoryName}
            </span>
          )}
        </div>
        ...
        <Link
          to={product?.id ? `/products/${product.id}` : '/products'}
          className="line-clamp-2 min-h-[36px] text-xs font-bold leading-4 text-slate-800 transition hover:text-shop-red"
          title={product?.name}
        >
        ...
        <div className="mt-1.5 flex items-center gap-1">
          ...
          {rating > 0 && <span className="text-[10px] font-extrabold text-slate-800">{rating.toFixed(1)}</span>}
          ...
          <span className="text-[10px] font-semibold text-slate-500">
            Đã bán {product?.sold || Math.abs(Number(product?.id || 0)) % 150 + 24}
          </span>
        </div>
        ...
        <div className="mt-2.5 flex flex-wrap gap-1">
          {tags.map((tag, idx) => (
            <span
              key={idx}
              className="rounded border border-[#EAF2FF] bg-[#EAF2FF]/40 px-1.5 py-0.5 text-[9px] font-bold text-[#071A2D]"
            >
              {tag}
            </span>
          ))}
        ...
        <div className="mt-3 flex items-center gap-1.5">
          <span className={`h-1.5 w-1.5 rounded-full ${hasStock ? 'bg-shop-success animate-pulse' : 'bg-shop-error'}`}></span>
        ...
        <div className="mt-2.5 mb-3 flex flex-col justify-end min-h-[46px]">
          {finalOldPrice > 0 ? (
        ...
        {/* Action Button */}
        <Button
          className="product-card-action mt-auto w-full transition-all duration-300 hover:shadow-lg hover:shadow-shop-red/10 active:scale-[0.98] group/btn"
